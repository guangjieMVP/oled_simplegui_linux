...
#include<time.h>
#else // _SIMPLE_GUI_IN_VIRTUAL_SDK_
#include "sgui_port.h"
#endif
...
#else
    struct simplegui_rtc rtc;
    RTCTimerEnable(true);
    sgui_rtc_get_time(&rtc);
    HMI_DemoPolarCoord_PaintClock(pstDeviceIF, rtc.hour, rtc.min, rtc.sec);
    //HMI_DemoPolarCoord_PaintClock(pstDeviceIF, 15, 40, 20);
    HMI_DemoPolarCoord_PaintClockText(pstDeviceIF, rtc.hour, rtc.min, rtc.sec);
#endif
...
static void HMI_DemoPolarCoord_PaintClockText(SGUI_SCR_DEV* pstDeviceIF, SGUI_INT iHour, SGUI_INT iMinute, SGUI_INT iSecond)
{
    ...
    SGUI_Text_DrawText(pstDeviceIF, szTextBuffer, &SGUI_DEFAULT_FONT_8, &stPaintArea, &stInnerPos, SGUI_DRAW_NORMAL);
}
